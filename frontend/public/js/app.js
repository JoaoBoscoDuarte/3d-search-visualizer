...
function getSelectedCompareAlgorithms() {
  return [...document.querySelectorAll('.compare-algo:checked')].map((el) => el.value);
}

function getCompareLimits() {
  const limits = {};
  const dls = Number($('compare-limit-dls')?.value || $('dls-limit')?.value || 80);
  const ids = Number($('compare-limit-ids')?.value || $('dls-limit')?.value || 80);
...
function setViewMode(mode) {
  ...
  if (btnCompare) btnCompare.style.display = isCompare ? 'none' : 'inline-block';
  const compareSetup = $('compare-setup');
  if (compareSetup) compareSetup.style.display = isCompare ? 'none' : 'inline-block';
...
async function init() {
  try {
    ...
    if (btn) {
      btn.disabled = true;
      btn.title = 'Módulo de comparação não carregou — verifique o console (F12)';
    }
  }
